...
(function () {
    window.onload = function () {
        ...
        window.benchmarkClient.willRunTest = function (suite, test) {
            ...
            const results = {benchmark: "Speedometer2", suite: suite.name, test: test.name};
        ...
        const originalDidRunSuites = window.benchmarkClient.didRunSuites;
        window.benchmarkClient.didRunSuites = function (measuredValues) {
            originalDidRunSuites.call(this, measuredValues);
            const xhr = new XMLHttpRequest();
            xhr.open("POST", "/IterationComplete");
            xhr.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
            const results = {benchmark: "Speedometer2", results: measuredValues};
            xhr.send(JSON.stringify(results));
        }

        const didFinishLastIteration = window.benchmarkClient.didFinishLastIteration;
        window.benchmarkClient.didFinishLastIteration = function (measuredValues) {
            didFinishLastIteration.call(this, measuredValues);
            const xhr = new XMLHttpRequest();
            xhr.open("POST", "/BenchmarkComplete");
            xhr.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
            xhr.send(JSON.stringify({benchmark: "Speedometer2"}));
        }
    }
})();
