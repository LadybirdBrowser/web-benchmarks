...
(function () {
    window.__webKitPerfTestState = { done: false, error: null, values: null, unit: null };
    ...
    function logInDocument(text) {
        ...
        window.scrollTo(0, document.body ? document.body.height : 0);
    }
    ...
    function logFatalError(text) {
        window.__webKitPerfTestState.error = text;
    ...
    function start(test, runner, doNotLogStart) {
        ...
        var requestedIterationCount = parseInt(new URLSearchParams(location.search).get("iterationCount"));
        iterationCount = test.customIterationCount || (requestedIterationCount > 0 ? requestedIterationCount : 1);
        var requestedRunCount = parseInt(new URLSearchParams(location.search).get("runCount"));
        runsPerIteration = requestedRunCount > 0 ? requestedRunCount : 1;
    ...
    function finish() {
        try {
            ...
            window.__webKitPerfTestState.error = "Got an exception while finalizing the test with name=" + exception.name + ", message=" + exception.message;
        ...
        if (!currentTest.continueTesting) {
            window.__webKitPerfTestState.values = results.slice();
            window.__webKitPerfTestState.unit = PerfTestRunner.unit;
            window.__webKitPerfTestState.done = true;
            if (window.testRunner)
    ...
    PerfTestRunner.measureRunsPerSecond = function (test) {
        PerfTestRunner.unit = "ms";
        start(test, measureFixedRunCountOnce);
    }

    function measureFixedRunCountOnce() {
        return callRunAndMeasureTime(runsPerIteration);
    }

    function callRunAndMeasureTime(runCount) {
